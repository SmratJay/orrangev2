...
function DitheredWaves({ waveSpeed, waveFrequency, waveAmplitude, waveColor, colorNum, pixelSize, disableAnimation, enableMouseInteraction, mouseRadius }: DitheredWavesProps) {
  ...
  useFrame(({ clock }) => {
    ...
    if (!disableAnimation) (u.time as THREE.Uniform<number>).value = clock.getElapsedTime();
    if ((u.waveSpeed as THREE.Uniform<number>).value !== waveSpeed) (u.waveSpeed as THREE.Uniform<number>).value = waveSpeed;
    if ((u.waveFrequency as THREE.Uniform<number>).value !== waveFrequency) (u.waveFrequency as THREE.Uniform<number>).value = waveFrequency;
    if ((u.waveAmplitude as THREE.Uniform<number>).value !== waveAmplitude) (u.waveAmplitude as THREE.Uniform<number>).value = waveAmplitude;
    if (!prevColor.current.every((v, i) => v === waveColor[i])) {
      (u.waveColor as THREE.Uniform<THREE.Color>).value.set(...waveColor);
    ...
    if (enableMouseInteraction) (u.mousePos as THREE.Uniform<THREE.Vector2>).value.copy(mouseRef.current);
  });
